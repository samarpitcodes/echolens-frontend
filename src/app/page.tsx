...
function MetricCard({ icon: Icon, label, value, detail, tone }: { icon: typeof FileText; label: string; value: string; detail: string; tone: string }) {
  return <article className="metric-card">
    <div className={`metric-icon ${tone}`}><Icon size={20} /></div>
    <div><p>{label}</p><strong>{value}</strong></div>
    <span className="metric-change">{detail}</span>
...
function DocumentDistribution({ projects }: { projects: Project[] }) {
  const entries = projects.slice(0, 6);
  const maxDocuments = Math.max(1, ...entries.map((project) => project.document_count ?? 0));
  if (!entries.length) return <p className="empty-text">Create a project to see live workspace data.</p>;
  return <div className="document-distribution">{entries.map((project) => { const count = project.document_count ?? 0; return <div className="document-bar" key={project.id}><div><span title={project.name}>{project.name}</span><strong>{count}</strong></div><i><b style={{ width: `${Math.max(3, (count / maxDocuments) * 100)}%` }}/></i></div>; })}</div>;
}
...
export default function HomePage() {
  ...
  useEffect(() => {
    listProjects().then((result) => setProjects(result.items)).catch(() => setProjects([])).finally(() => setLoading(false));
  }, []);
  ...
  const documentCount = projects.reduce((total, project) => total + (project.document_count ?? 0), 0);
  ...
  return <div className="app-shell">
    ...
    <main className="dashboard-main">
      ...
      <section className="page-intro"><div><h1>{greeting}, {name.split(" ")[0]} <span>👋</span></h1><p>Here&apos;s what&apos;s happening with your projects today.</p></div><CreateProjectDialog onCreated={(project) => setProjects((current) => [project, ...current])} /></section>

      <section className="metrics"><MetricCard icon={FolderKanban} label="Total Projects" value={String(projects.length)} detail="Live project count" tone="violet"/><MetricCard icon={FileText} label="Documents" value={String(documentCount)} detail="Across your projects" tone="blue"/><MetricCard icon={MessageSquareText} label="AI Chats" value="—" detail="Chat analytics unavailable" tone="teal"/><MetricCard icon={Globe2} label="Sources Used" value="—" detail="Source analytics unavailable" tone="purple"/></section>
      ...
      <section className="dashboard-grid">
        ...
        <article className="panel activity-panel"><div className="panel-heading"><h2>Documents by project</h2><button className="period-button" onClick={() => setPeriod(period === "This Week" ? "All projects" : "This Week")}>{period} <ChevronDown size={14}/></button></div><DocumentDistribution projects={projects} /></article>
